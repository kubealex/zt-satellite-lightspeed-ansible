...
# This script runs as root on aap1 (via runtime-automation/main.yml, as
# module-04's "setup" stage). It assumes module-03/setup-aap1.sh already
# ran (self-hosted rulebook repo, EDA credentials/Project/Event
# Stream/Activation, aap1-user's cross-host SSH trust) - this script
# only adds what Module 4 (Close the Loop: Automatic Remediation)
# itself needs: the custom registry auth, the vulnerability-remediation
# repo, and the two scripts Module 4's Steps 4/5 walk the participant
# through running.
#
# This used to live in setup-automation/setup-aap1.sh (run once for the
# whole lab before any module); it moved here so each module's own
# runtime-automation setup script only contains what that module
# actually needs.
...
# through by runtime-automation/main.yml.
#
...
# assumes that user's $HOME.
sudo -u aap1-user -H bash <<'AAPUSER_EOF'
set -e
# The outer 'sh -x .../setup-aap1.sh > setup-aap1.log 2>&1' invocation
# (see runtime-automation/main.yml) only traces this outer script's own
...
echo "==> Ensure the vulnerability finder + remediation playbook repo"
# Self-hosted the same way as the satellite-webhook repo from Module 3,
# reusing the same deploy key. Pre-populating this here (rather than
# having the participant paste an ~800-line script and a playbook by
# hand in Module 4) removes the biggest source of copy/paste errors in
# that module - Module 4 only explains what each file does and runs
# commands against them, it never creates them.
mkdir -p ~/git
...
echo "==> SETUP-MODULE-04-AAP1 (repo pre-population): SUCCESS"
AAPUSER_EOF
...
# throughout module-03/setup-aap1.sh), so they don't depend on any shell
# variables from earlier steps in the participant's own terminal session
# - only on resources that already exist by the time each step runs.
# Writing these here (rather than having the participant paste large
# heredocs that write intermediate helper files, e.g. a Python payload
# builder or the rulebook YAML, by hand) removes that copy/paste risk
# the same way the vulnerability-remediation repo pre-population above
# already does - Module 4 only explains what each script does and runs
# it, it never creates one from scratch in front of the participant.
...
echo "==> SETUP-MODULE-04-AAP1: SUCCESS"
