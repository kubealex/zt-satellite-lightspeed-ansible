...
# module-03's "setup" stage). AAP itself, and the aap1-user OS account,
# are already baked into the aap1-* golden image - this script only
# wires up the Satellite -> EDA webhook pipeline (self-hosted rulebook
# repo, EDA credentials, Project, Event Stream, Activation) idempotently.
...
# actually needs, keeping any single file shorter and the dependency
# between "module N's content" and "module N's setup" explicit.
#
...
# This module's own solve script (and its Step 1/3 instructions) need
...
echo "==> SETUP-MODULE-03-AAP1: SUCCESS"
AAPUSER_EOF
